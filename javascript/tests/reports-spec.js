...
//       expect(defaultFiltersState.allItemsMap['abc__catA']).not.eq(undefined)
//       expect(defaultFiltersState.allItemsMap['abc__catA'][`${testExpiration}__null`][0].quantity).eq(1)
//       expect(defaultFiltersState.allItemsMap['bcd__catA'][`null__abc`].length).eq(2)
//     }
//   },
//   'Running a report': {
//     'should return report rows and report headers' () {
//       expect(consumptionState.displayRows instanceof Array).eq(true)
//       expect(consumptionState.displayHeaders instanceof Array).eq(true)
//     },
//     'should default to batch level' () {
//       expect(consumptionState.displayRows.length).gt(5)
//     },
//     'should filter out batch rows with all zeros but leave one row at the item level' () {
//       expect(consumptionState.displayRows.length).eq(6)
//     },
//     'should order alphabetically' () {
//       const expectedOrder = ['abc', 'bcd', 'bcd', 'cde', 'def', 'efg']
//       consumptionState.displayRows.forEach((row, i) => {
...
//       expect(consumptionStateItemLevel.displayRows.length).eq(5)
//       expect(consumptionStateItemLevel.displayRows[0].expiration).eq(undefined)
//       expect(consumptionStateItemLevel.displayRows[1].opening).eq(3)
//     }
//   },
//   'Running a report with category filters': {
//     'should only return rows for the specified category' () {
//       expect(consumptionStateCategoryFilter.displayRows.length).eq(4)
//       expect(consumptionStateCategoryFilter.displayRows[0].category).eq('catA')
//       const catBState = reportsReducer(defaultFiltersState, runReportAction('consumption', 'categories', 2))
//       expect(catBState.displayRows.length).eq(1)
//       expect(catBState.displayRows[0].category).eq('catB')
//     }
//   }
//
// }
